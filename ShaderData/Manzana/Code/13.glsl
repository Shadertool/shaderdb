...
vec2 rand(vec3 co){
    return vec2(
	    fract(sin(dot(co.xyz,vec3(27.2344,98.2142,57.2324)))*43758.5453),
	    fract(cos(dot(co.xyz,vec3(34.7483,42.8534,12.1234)))*53978.3542));
}
...
vec3 camera(
  ...
  in float vpd)
{
  vec2 rnd=rand(vec3(gl_FragCoord.xy,time));
...
void main(void){
  ...
  gl_FragColor=vec4(c1.xyz*0.75+c2.xyz*0.25,1.0)*(1.0/maxaa)+backpixel*((maxaa-1.0)/maxaa);
